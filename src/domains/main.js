...
class MainDomain {
	...
	create() {
		...
		this.game.state.add("5", new Level(this, 4, 4, 100, 4, 0.7, 8, "1"));
...
class Level {
    ...
    */
	checkSize(numRows, numCols, tileSize, tileSpacing) {
		return (
			(numCols * tileSize + numCols * tileSpacing) / this.game.width + 1
		);
	}
	...
	placeTiles(numRows, numCols) {
		...
		var leftSpace =
			(this.game.width -
				numCols * (tileSize * this.tileScaling) -
				tileSpacing) /
			2;
		var topSpace =
			(this.game.height -
				numRows * (tileSize * this.tileScaling) -
				tileSpacing) /
			2;
		...
		for (i = 0; i < numCols; i++) {
			for (var j = 0; j < numRows; j++) {
				var tile = this.game.add.button(
					leftSpace +
						i * (this.tileSize * this.tileScaling + tileSpacing),
					topSpace +
						j * (this.tileSize * this.tileScaling + tileSpacing),
					"tiles",
					this.showTile,
					this
				);
	...
	showTile(target) {
		if (
			this.selectedArray.length < 2 &&
			this.selectedArray.indexOf(target) == -1
		) {
